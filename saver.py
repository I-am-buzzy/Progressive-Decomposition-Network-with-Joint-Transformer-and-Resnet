...
def resume(model, optimizer=None, model_save_path=None, device=None, is_train=True):  # resume(PSF, optimizer=None, model_save_path=None, device=None, is_train=True)
    # weight
...
class Saver(nn.Module):
    def __init__(self, opts):
        ...
        self.semanic_dir = os.path.join(self.model_dir, 'semanic')
        self.model_dir = os.path.join(self.model_dir, 'checkpoints')

        # make directory
        if os.path.exists(self.display_dir):
            shutil.rmtree(self.display_dir)  # shutil.rmtree() #递归地删除文件
        os.makedirs(self.display_dir, exist_ok=True)  # 用来创建多层目录
        if not os.path.exists(self.model_dir):
            os.makedirs(self.model_dir)
        if not os.path.exists(self.semanic_dir):
            os.makedirs(self.semanic_dir)
    ...
    # save result images
    def write_img(self, ep, input, output):  # input = [img_ir, img_vi, fused_img, label]   ([4, 1, 256, 256])
        input = torch.cat((input[0][:1, ::].repeat(1, 3, 1, 1), input[1][:1, ::], input[2][:1, ::].repeat(1, 3, 1, 1),
                           input[3][:1, ::].repeat(1, 3, 1, 1)), 3)  # .repeat是复制维数([4, 3, 256, 256])
    ...
    def write_img1(self, ep, input, output):      #([4, 1, 256, 256])
        input = torch.cat((input[0][:1, ::].repeat(1, 3, 1, 1), input[1][:1, ::], input[2][:1, ::].repeat(1, 3, 1, 1),
                           input[3][:1, ::].repeat(1, 3, 1, 1)), 3)        # .repeat是复制维数([4, 3, 256, 256])
        pred = torch.cat((output[0][:1, ::].repeat(1, 3, 1, 1), output[1][:1, ::].repeat(1, 3, 1, 1), output[2][:1, ::],
                          output[3][:1, ::].repeat(1, 3, 1, 1)), 3)
        assembled_images = torch.cat((input, pred), 2)
        img_filename = '%s/%05d.jpg' % (self.semanic_dir, ep)
        logger.info('Save model to: {}'.format(img_filename))
        torchvision.utils.save_image(assembled_images, img_filename, nrow=1)
    ...
    def write_model(self, ep, total_it, model, optimizer, best_mIoU, device, is_best=True, logger=None):
        model = model.cpu()
        state = {'MTAN': model.state_dict(), 'optimizer': optimizer.state_dict(), 'ep': ep, 'total_it': total_it, 'best_mIou': best_mIoU}
