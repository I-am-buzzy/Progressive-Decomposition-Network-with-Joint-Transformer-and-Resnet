...
def randrot(img):
	mode = np.random.randint(0, 4)   #用于生成随机整数0,1,2，3
	return rot(img, mode)


def randfilp(img):      #torch.Size([1, 8, 480, 640])
	mode = np.random.randint(0, 3)    #用于生成随机整数0,1,2
	return flip(img, mode)


def rot(img, rot_mode):
	if rot_mode == 0:
		img = img.transpose(-2, -1)   #torch.Size([1, 8, 640, 480])
		img = img.flip(-2)   #翻转
	elif rot_mode == 1:
		img = img.flip(-2)
		img = img.flip(-1)
		...
		img = img.flip(-2)
		img = img.transpose(-2, -1)    #torch.Size([1, 8, 480, 640])
	return img
...
def RGB2YCrCb(rgb_image):     #将RGB格式转换为YCrCb格式，用于中间结果的色彩空间转换中,因为此时rgb_image默认size是[B, C, H, W] :param rgb_image: RGB格式的图像数据  :return: Y, Cr, Cb
	R = rgb_image[:, 0:1]
	G = rgb_image[:, 1:2]
	B = rgb_image[:, 2:3]
...
def YCbCr2RGB(Y, Cb, Cr):    #将YcrCb格式转换为RGB格式 param： Y， Cb， Cr
...
def logger_config(log_path, logging_name):  # 配置log， param log_path: 输出log路径， param logging_name: 记录中name,可随意
...
class SegmentationMetric(object):
	def __init__(self, numClass, device):    #(9,GPU)
	...
	def meanPixelAccuracy(self):   #Mean Pixel Accuracy(MPA，均像素精度)：是PA的一种简单提升，计算每个类内被正确分类像素数的比例，之后求所有类的平均。
	...
	def IntersectionOverUnion(self):
		...
		union = torch.sum(self.confusionMatrix, axis=1) + torch.sum(self.confusionMatrix, axis=0) - torch.diag(self.confusionMatrix)     # axis = 1表示混淆矩阵行的值，返回列表； axis = 0表示取混淆矩阵列的值，返回列表
	...
	def genConfusionMatrix(self, imgPredict, imgLabel, ignore_labels):  #同FCN中score.py的fast_hist()函数,计算混淆矩阵，return: 混淆矩阵
	...
	def Frequency_Weighted_Intersection_over_Union(self):     #FWIoU, 频权交并比:为MIoU的一种提升, 这种方法根据每个类出现的频率为其设置权重。 FWIOU =  [(TP+FN)/(TP+FP+TN+FN)] *[TP / (TP + FP + FN)]
		freq = torch.sum(self.confusion_matrix, axis=1) / torch.sum(self.confusion_matrix)
		iu = np.diag(self.confusion_matrix) / (torch.sum(self.confusion_matrix, axis=1) + torch.sum(self.confusion_matrix, axis=0) - torch.diag(self.confusion_matrix))
...
if __name__ == '__main__':
	...
	metric = SegmentationMetric(3, device)     # 3表示有3个分类，有几个分类就填几, 0也是1个分类
