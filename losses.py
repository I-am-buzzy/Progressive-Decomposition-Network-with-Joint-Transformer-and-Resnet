...
def Fusion_loss(ir, vi, fu, mask, weights=[1, 10, 10], device=None):   #Fusion_loss(vi_Y, img_ir, fused_img, mask=mask, device=device)
    # grad_ir =  KF.spatial_gradient(ir, order=2).abs().sum(dim=[1,2])
    # grad_vi = KF.spatial_gradient(vi, order=2).abs().sum(dim=[1,2])
    # grad_fus = KF.spatial_gradient(fu, order=2).abs().sum(dim=[1,2])
    # grad_joint = torch.max(grad_ir, grad_vi)
    sobelconv=Sobelxy(device) 
    vi_grad_x, vi_grad_y = sobelconv(vi)   #torch.Size([4, 1, 256, 256])  torch.Size([4, 1, 256, 256])
    ...
    loss_grad = F.l1_loss(grad_joint_x, fu_grad_x) + F.l1_loss(grad_joint_y, fu_grad_y)   # 梯度损失
    # loss_grad = F.l1_loss(grad_fus, grad_joint)
    loss_ssim = corr_loss(ir, vi, fu)      #SSIM损失
    loss_intensity = final_mse1(ir, vi, fu, mask) + 0 * F.l1_loss(fu, torch.max(ir, vi))     #强度损失
    loss_total = weights[0] * loss_ssim + weights[1] * loss_grad + weights[2] * loss_intensity
...
class Fusionloss(nn.Module):
    ...
    def forward(self, vi, ir, fu, mask=None):
        ...
        # temp=(image_ir*x_y_ +origin)/(1.0-image_ir)+x_y_origin
...
class Sobelxy(nn.Module):
    ...
    def forward(self, x):   #torch.Size([4, 1, 256, 256])  self.weightx.shape:[1, 1, 3, 3]
        sobelx = F.conv2d(x, self.weightx, padding=1)   #torch.Size([4, 1, 256, 256])
        sobely = F.conv2d(x, self.weighty, padding=1)   #torch.Size([4, 1, 256, 256])
...
def cc(img1, img2):
    ...
    N, C, _, _ = img1.shape
    img1 = img1.reshape(N, C, -1)   #(8,1,256*256)
    img2 = img2.reshape(N, C, -1)
    img1 = img1 - img1.mean(dim=-1, keepdim=True)      #(8,1,1), (8,1,256*256)
    img2 = img2 - img2.mean(dim=-1, keepdim=True)
    cc = torch.sum(img1 * img2, dim=-1) / (eps + torch.sqrt(torch.sum(img1 ** 2, dim=-1)) * torch.sqrt(torch.sum(img2 ** 2, dim=-1)))
    cc = torch.clamp(cc, -1., 1.)    #将输入input张量每个元素的范围限制到区间 [min,max]，返回结果到一个新张量。
    return cc.mean()


def Re_loss(img1, img2, weights=None, mask=None, ir_flag=False):   # img1 is the reconstructed image, img2 is the reference image
    if weights is None:
        weights = [5, 5]
...
def Seg_loss(pred, label, device, criteria=None):  #Seg_loss(seg_pred, label, device, criteria)   #criteria = OhemCELoss  交叉熵损失
    '''
    ...
    seg_loss = criteria(pred, lb)    #criteria = OhemCELoss
    # lb = torch.squeeze(label, 1)
...
def l1loss(img1,img2,mask=1,eps=1e-2):
    ...
    img1 = KF.gaussian_blur2d(img1, [3, 3], [1, 1])*mask_    #Create an operator that blurs a tensor using a Gaussian filter.
...
def feat_loss(feat1, feat2, grid=16):
    ...
    shift_x = np.random.randint(1, w//grid)    #用于生成随机整数
    ...
    perseed = torch.randperm(featset.shape[0])    #.randperm(n)：将0~n-1（包括0和n-1）随机打乱后获得的数字序列
