...
def main():
    ...
    MPF_model = PDFN(opts.class_nb).to(device)
    MPF_model = resume(MPF_model, model_save_path=opts.resume, device=device, is_train=False)
    
    # define dataset    
    test_dataset = FusionData(opts)
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=opts.batch_size, shuffle=False)
...
def multi_task_tester(test_loader, multi_task_model, device, opts):
    ...
    #Fusion_save_dir1 = os.path.join(opts.result_dir1, opts.dataname)
    os.makedirs(Fusion_save_dir, exist_ok=True)
    #os.makedirs(Fusion_save_dir1, exist_ok=True)
    with torch.no_grad():         #operations inside don't track history
        for it, (img_ir, img_vi, img_names, widths, heights) in enumerate(test_bar):
            ...
            fused_img = YCbCr2RGB(fused_img, vi_Cb, vi_Cr)   #融合后的结果图像是彩色的三通道
            #fused_img1 = YCbCr2RGB(img_fuse, vi_Cb, vi_Cr)

            for i in range(len(img_names)):
                img_name = img_names[i]
                fusion_save_name = os.path.join(Fusion_save_dir, img_name)
                #fusion_save_name1 = os.path.join(Fusion_save_dir1, img_name)
                save_img_single(fused_img[i, ::], fusion_save_name, widths[i], heights[i])
                #save_img_single(fused_img1[i, ::], fusion_save_name1, widths[i], heights[i])
